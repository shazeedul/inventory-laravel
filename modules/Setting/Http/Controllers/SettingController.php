...
class SettingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $group = $request->g ?? 'general';

        $settings = $settings->where('group', $group);
        $groupName = Str::lower($group);
        // all space replace with underscore
        $groupName = implode('_', explode(' ', $groupName));
        if (view()->exists('setting::setting.'.$groupName)) {
            return \view('setting::setting.'.$groupName, [
