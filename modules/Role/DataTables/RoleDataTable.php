    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                return '<a href="'.route(config('theme.rprefix').'.edit', $query->id).'" class="btn btn-primary btn-sm me-1" title="Edit"><i class="fa fa-edit"></i></a>'.
                    '<a href="#" class="btn btn-danger btn-sm" onclick="delete_modal(\''.route(config('theme.rprefix').'.destroy', $query->id).'\',\'role-table\')"  title="Delete"><i class="fa fa-trash"></i></a>';
            })
            ->setRowId('id')
            ->addIndexColumn();
    }
