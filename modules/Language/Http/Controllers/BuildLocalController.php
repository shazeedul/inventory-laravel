...
class BuildLocalController extends Controller
{
    ...
    {
        cs_set('theme', [
            'title' => ___('Translate') . $language->title . ' (' . $language->code . ')' . ___('Language'),
        ]);
    ...
    public function translate($code, Request $request)
    {
        ...
        try {
            ...
            return response()->error([], ___("Local translate not possible."));
        }
        return response()->success([], ___('Local translate successfully'));
