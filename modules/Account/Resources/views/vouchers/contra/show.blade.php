...
<div class="" style="padding: 50px 0">
    <div class="row gutters-sm m-3">
        <div class="col-md-12" id="print-table-{{ $contra->id }}">
            <div class="row">
            ...
            <table class="table table-bordered table-sm mt-2 voucher">
                ...
                <tbody>
                    @foreach ($contra->all_vouchers_by_no as $voucher)
                        <tr>
                            <td>
                    ...
                    <tr>
                        <td>
                            <strong>{{ $contra->reverse_code ? $contra->reverseCode?->name : ' ' }}</strong>
                            <br>
                            @if ($contra->reverseCode?->is_bank_nature)
                                @if ($contra->cheque_no)
                                    <span>@localize('check_no'): {{ $contra->cheque_no }}, </span>
                                @endif
                                @if ($contra->cheque_date)
                                    <span>@localize('check_date'): {{ $contra->cheque_date }}</span>
                                @endif
                            @endif
                        </td>

                        <td class="text-end">
                            {{ $contra->all_vouchers_by_no->sum('credit') }}
                        </td>
                        <td class="text-end">0</td>
                    </tr>
                ...
                <tfoot>
                    ...
                    <tr>
                        <th class="" colspan="3">@localize('in_words'):
                            {{-- {{ numberToWords($contra->all_vouchers_by_no->sum('credit')) }} --}}
                        </th>
                    </tr>
                    <tr>
...
<div class="modal-footer d-print-none">
    ...
    {{-- <button type="button" onclick="printVoucher('print-table-{{ $contra->id }}')"
