...
class ContraVoucherDataTable extends DataTable
{
    ...
    public function dataTable($query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $button = '';
                $button .= '<a href="javascript:void(0);"  onclick="' . "axiosModal('" . route('admin.account.voucher.contra.show', $query->id) . '\')" title="' . localize('Show') . '" class="btn btn-primary btn-sm me-2"><i class="fas fa-eye"></i></a>';
                if (!$query->is_approved) {
                    $button .= '<a href="' . route('admin.account.voucher.contra.edit', $query->id) . '" class="btn btn-secondary btn-sm me-2"><i class="fas fa-edit"></i></a>';
                    $button .= '<a href="javascript:void(0);" class="btn btn-danger btn-sm" onclick="' . "delete_modal('" . route('admin.account.voucher.contra.destroy', $query->id) .  '\')"  title="' . localize('Delete') . '"><i class="fa fa-trash"></i></a>';
                } else {
    ...
    {
        return $model->newQuery()->with(['chartOfAccount', 'reverseCode', 'accountSubType'])->where('account_voucher_type_id', 3)->orderBy('voucher_no', 'DESC');
    }
