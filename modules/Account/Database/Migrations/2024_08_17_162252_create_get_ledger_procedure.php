...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("
            ...
            BEGIN
                ...
                DECLARE account_voucher_type_id INT;
                ...
                DECLARE transaction_cursor CURSOR FOR
                    SELECT t.id, t.voucher_no, t.voucher_date, IFNULL(t.narration, '') AS narration,
                        IFNULL(t.debit, 0.00) AS debit, IFNULL(t.credit, 0.00) AS credit, t.account_voucher_type_id,
                        coas.name AS reverse_account_name
                    ...
                    AND t.voucher_date BETWEEN 
                        (SELECT start_date FROM financial_years AS f WHERE f.status = 1 AND f.start_date <= start_date ORDER BY f.start_date DESC LIMIT 1)
                        AND end_date
                    ORDER BY t.voucher_date ASC, t.id ASC;

                DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

                CREATE TEMPORARY TABLE IF NOT EXISTS temp_table_ld (
                    voucher_id INT,
                    voucher_date DATE,
                    voucher_no VARCHAR(191),
                    account_voucher_type_id INT,
                    reverse_account_name VARCHAR(191),
                    voucher_remarks TEXT,
                    voucher_debit DOUBLE,
                    voucher_credit DOUBLE,
                    voucher_balance DOUBLE
                );

                IF EXISTS (SELECT 1 FROM account_opening_balances WHERE chart_of_account_id = chart_of_account_id) THEN
                    SELECT
                        CASE
                        ...
                        o.financial_year_id
                    INTO
                        opening_date, opening_debit, opening_credit, opening_financial_year_id
                ...
                IF get_account_nature = 'DR' THEN
                    SET opening_balance = IFNULL(opening_debit, 0.00) - IFNULL(opening_credit, 0.00);
                ELSEIF get_account_nature = 'CR' THEN
                    SET opening_balance = IFNULL(opening_credit, 0.00) - IFNULL(opening_debit, 0.00);
                END IF;

                INSERT INTO temp_table_ld (
                    voucher_id, voucher_date, voucher_no, account_voucher_type_id, reverse_account_name, voucher_remarks, voucher_debit, voucher_credit, voucher_balance
                ) VALUES (
                    0, opening_date, 'Opening Balance', 0, '', 'Opening Balance', 0.00, 0.00, opening_balance
                );
                ...
                transaction_loop: LOOP
                    FETCH transaction_cursor INTO 
                        voucher_id, voucher_no, transaction_date, voucher_remarks, transaction_debit, transaction_credit, account_voucher_type_id, reverse_account_name;
                    IF done THEN
                    ...
                    ELSEIF get_account_nature = 'CR' THEN
                        SET opening_balance = opening_balance + IFNULL(transaction_credit, 0.00) - IFNULL(transaction_debit, 0.00);
                    END IF;

                    INSERT INTO temp_table_ld (
                        voucher_id, voucher_date, voucher_no, account_voucher_type_id, reverse_account_name, voucher_remarks, voucher_debit, voucher_credit, voucher_balance
                    ) VALUES (
                        voucher_id, transaction_date, voucher_no, account_voucher_type_id, reverse_account_name, voucher_remarks, transaction_debit, transaction_credit, opening_balance
                    );
                END LOOP;
                ...
                INSERT INTO temp_table_ld (
                    ...
                    0, end_date, 'Closing Balance', 0, '', 'Closing Balance', 0.00, 0.00, opening_balance
                );

                SELECT * FROM temp_table_ld;

                DROP TABLE IF EXISTS temp_table_ld;
            END;
        ");
    }
    ...
    {
        DB::statement('DROP PROCEDURE IF EXISTS GetLedger');
    }
};
