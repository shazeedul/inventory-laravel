...
trait DataTableActionBtn
{
    ...
    ])
    {
        ...
        if (isset($option['show'])) {
            if ($option['show'] === true) {
                $actionBtn .= '<a href="javascript:void(0);" class="btn btn-primary btn-sm my-1 mx-1" onclick="'."axiosModal('".route(config('theme.rprefix').'.show', $this->id).'\')"><i class="fa fa-eye"></i></a>';
            } else {
        ...
        if (isset($option['delete'])) {
            if ($option['delete'] === true) {
                $actionBtn .= '<a href="javascript:void(0);" class="btn btn-danger btn-sm" onclick="'."delete_modal('".route(config('theme.rprefix').'.destroy', $this->id).'\')"  title="'.__('Delete').'"><i class="fa fa-trash"></i></a>';
            } else {
