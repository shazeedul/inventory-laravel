...
{
    if ($file) {
        return app('url')->asset($path.'/'.$file);
    }
...
function get_role($user = null)
{
    if ($user != null) {
        return $user->roles()->select(['id', 'name', 'guard_name'])->get();
    }

    return auth()->user()->roles()->select(['id', 'name', 'guard_name'])->get();
}
 ...
 * @param  mixed  $initPath
 * @param  mixed  $manifest
 * @return string
 *
 * @throws \Exception
 */
function vite($entrypoint, $initPath = 'build/', $manifest = null)
{
    ...
    if (\Illuminate\Support\Facades\File::exists($manifest)) {
        ...
        if (is_array($entrypoint)) {
            foreach ($entrypoint as $item) {
                $path = $manifest[$item] ?? $item;
                if (! is_array($path)) {
                    // check if it is css
                    if (pathinfo($path, PATHINFO_EXTENSION) == 'css' || pathinfo($path, PATHINFO_EXTENSION) == 'css.map' || pathinfo($path, PATHINFO_EXTENSION) == 'scss') {
                    ...
                    //    if path[file] is css
                    if (pathinfo($path['file'], PATHINFO_EXTENSION) == 'css' || pathinfo($path['file'], PATHINFO_EXTENSION) == 'css.map' || pathinfo($path['file'], PATHINFO_EXTENSION) == 'scss') {
                        echo '<link rel="preload" as="style"  href="'.asset($initPath.$path['file']).'" />';
                        echo '<link rel="stylesheet" href="'.asset($initPath.$path['file']).'" />';

                    } else {

                        echo '<script type="module" src="'.asset($initPath.$path['file']).'"></script>';
                        echo '<link rel="modulepreload" href="'.asset($initPath.$path['file']).'" />';
                    }
                    if (array_key_exists('css', $path)) {
                        foreach ($path['css'] as $css) {
                            echo '<link rel="preload" as="style" href="'.asset($initPath.$css).'" />';
                            echo '<link <link rel="stylesheet" href="'.asset($initPath.$css).'" />';
                        }
                    }
                }
...
/**
 * To display diff for humans date
 *
...
{
    $carbon = new \Carbon\Carbon($timestamp);
...
/**
 * Convert string to limit
 *
 * @param  int  $limit
 */
function str_limit(string $txt, $limit = 50): string
{
    return Str::limit($txt, $limit);
}
